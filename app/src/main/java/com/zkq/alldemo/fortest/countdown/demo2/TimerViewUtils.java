...
public class TimerViewUtils {
    ...
    private static TimerViewUtils timerUtils;
    private static ArrayList<CustomCountDown> timerList;

    public static TimerViewUtils getInstance(){
        if(timerUtils == null){
            timerUtils = new TimerViewUtils();
            timerList = new ArrayList<>();
        }
        return timerUtils;
    }

    public CustomCountDown getCustomCountDown(CustomCountDown customCountDown
            , CountdownBean bean, OnTimerListener listener){
        customCountDown.setCountdownBean(bean).setOnTimerListener(listener);
        timerList.add(customCountDown);
        return customCountDown;
    }

    public ArrayList<CustomCountDown> getTimerList() {
        return timerList;
    }
    ...
    public void cancelById(String id){
        ...
        for (int i = 0; i < getTimerList().size(); i++) {
            CustomCountDown zkqCountDownTimer = getTimerList().get(i);
    ...
    public void reStartById(String id){
        ...
        for (int i = 0; i < getTimerList().size(); i++) {
            CustomCountDown zkqCountDownTimer = getTimerList().get(i);
            if(id.equals(zkqCountDownTimer.getId())){
                zkqCountDownTimer.restart();
                break;
            }
        }
    }

    public void goonById(String id){

        for (int i = 0; i < getTimerList().size(); i++) {
            CustomCountDown zkqCountDownTimer = getTimerList().get(i);
            if(id.equals(zkqCountDownTimer.getId())){
                zkqCountDownTimer.goon();
                break;
            }
        }
    }
    ...
    public static String getTimerStr(String mTimerStr, String replace){
        return mTimerStr.replaceAll("[^\\d]",replace);
    }
