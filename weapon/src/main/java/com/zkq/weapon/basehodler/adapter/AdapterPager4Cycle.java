...
/**
 * @author zkq
 * create:2019/5/28 11:37 PM
 * email:zkq815@126.com
 * desc: ViewPager无限循环适配器，特点：
 *  * 1，提供 默认样式、默认当前页数
 *  * 2，自定义 分页样式、首次当前页数
 *  * 3，调用简单，外界只需调用相关{@link #init(List)} ()}即可
 *  * 4，实现了ViewPager.OnPageChangeListener监听
 *  *
 *  * 注意：
 *  * 1，构造函数中的ViewPager对象，通过它的getCurrentItem()获取选中的位置是不正确的，请调用
 *  * {@link #getCurrPosition()} <br/>
 *  * 2，指定翻页图片后导致页数样式怪异，请重写 {@link #initPageItemHintView(List, ViewGroup, int)} ()}
 *  * 3，销毁时，最好主动调用{@link #clear()}释放内存
 */
public abstract class AdapterPager4Cycle<DataType> extends AdapterPagerBase<DataType> implements
    ...
    public Object instantiateItem(ViewGroup container, int position) {
        View view = initItemViewFacade(container, position, getCount());
    ...
    public void initPageItemHintView(List<DataType> datasLists, ViewGroup itemHintContainer, int position) {
        ...
        //间距
        itmeHintlayoutParams.setMargins(10, 0, 0, 0);
    ...
    public final void onPageSelected(int position) {
        ...
        if (getCount() > 1) {
            ...
            if (position < 1) {
                ...
                return;
            } else if (position > mDataLists.size()) {
                // 末位之后，跳转到首位
                position = 1;
                mViewPager.setCurrentItem(position, false);
                return;
