...
import com.zkq.weapon.market.glide.glideimpl.transform.RoundedCornersTransformation;

import com.zkq.weapon.market.glide.config.CommonLoaderConfig;
...
public class GlideLoaderImpl implements ILoad<GlideLoaderConfig> {
    ...
    public void loadImage(@NonNull GlideLoaderConfig config) {
        ...
        if (glideRequests != null) {
            ...
            // bitmap型
            if (config.isBitmap) {
                if (config.resourceType == CommonLoaderConfig.RESOURCE_TYPE_URL) {
                    intoBitmapTarget(glideRequests.asBitmap().load(config.url).apply(requestOptions), config);
                } else if (config.resourceType == CommonLoaderConfig.RESOURCE_TYPE_URI) {
                    intoBitmapTarget(glideRequests.asBitmap().load(config.uri).apply(requestOptions), config);
                } else if (config.resourceType == CommonLoaderConfig.RESOURCE_TYPE_FILE) {
                    intoBitmapTarget(glideRequests.asBitmap().load(config.file).apply(requestOptions), config);
                } else if (config.resourceType == CommonLoaderConfig.RESOURCE_TYPE_RES_ID) {
